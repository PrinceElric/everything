...
def code_names_game():
    ...
    while not set(accepted_prop) == set(mots_verts):
        ...
        while True:
            ...
            if propositions in mots_rouges:
                cprint(f"You lost!", ROUGE)  # noqa: F405
                cprint(f"the answers were : {reponses}", VERT + SOULIGN2)  # noqa: F405
                sys.exit(f"See you next time, you had {pts}pts")
            if propositions in mots_verts and propositions in reponses:
                pts += 1
                cprint(f"You won 1pts, you are now to {pts}pts!", VERT)  # noqa: F405
                repetition_aceptation_rep(propositions)
                nb_good_answ += 1
            elif propositions in reponses:
                cprint(
                    f"{VERT}Very well answer but you didn't make points{RESET}", VERT  # noqa: F405
                )  # noqa: F405
                cprint(f"You have {pts}!", VERT)  # noqa: F405
                repetition_aceptation_rep(propositions)
                nb_good_answ += 1
            else:
    ...
    cprint("You won!", VERT + SOULIGN2 + GRAS + ITALIC)  # noqa: F405
